...
# Create bucket
method = 'PUT'
res = obsClient.createV2SignedUrl(method, bucketName, expires=3600)
doAction('Creating bucket', method, res['signedUrl'])
...
res = obsClient.createV2SignedUrl(method, bucketName, specialParam='cors', headers=headers)
doAction('Setting bucket cors', method, res['signedUrl'], res['actualSignedRequestHeaders'], content)

method = 'GET'
res = obsClient.createV2SignedUrl(method, bucketName, specialParam='cors')
doAction('Getting bucket cors', method, res['signedUrl'])

method = 'DELETE'
res = obsClient.createV2SignedUrl(method, bucketName, specialParam='cors')
doAction('Deleting bucket cors', method, res['signedUrl'])

# Put object
method = 'PUT'
content = 'Hello OBS'
headers = {'Content-Length': str(len(content))}
res = obsClient.createV2SignedUrl(method, bucketName, objectKey, headers=headers)
doAction('Creating object', method, res['signedUrl'], res['actualSignedRequestHeaders'], content)

# Get object
method = 'GET'
res = obsClient.createV2SignedUrl(method, bucketName, objectKey)
doAction('Getting object', method, res['signedUrl'])

# Set/Get object acl
method = 'PUT'
headers = {'x-amz-acl': 'public-read'}
res = obsClient.createV2SignedUrl(method, bucketName, objectKey, specialParam='acl', headers=headers)
doAction('Setting object acl', method, res['signedUrl'], res['actualSignedRequestHeaders'])

method = 'GET'
res = obsClient.createV2SignedUrl(method, bucketName, objectKey, specialParam='acl')
doAction('Getting object acl', method, res['signedUrl'])

# Delete object
method = 'DELETE'
res = obsClient.createV2SignedUrl(method, bucketName, objectKey)
doAction('Deleting object', method, res['signedUrl'])

# Delete bucket
method = 'DELETE'
res = obsClient.createV2SignedUrl(method, bucketName, expires=3600)
doAction('Deleting bucket', method, res['signedUrl'])
